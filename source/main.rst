...
* |ic_ruler_round| Кнопка линейки измерений (включается: :menuselect:`Настройки --> Карта --> Линейка измерений`);
* Шкала масштаба (в левом нижнем углу, включается: :menuselect:`Настройки --> Карта --> Шкала масштаба`).

В нижней части Главного окна приложения может размещаться **Панель статуса** (настроить отображение панели статуса можно в: :menuselect:`Настройки -> Карта -> Показать панель статуса`). В зависимости от размера экрана Панель статуса может занимать одну или две строки.
...
* текущий уровень приближения (включается в :menuselect:`Настройки --> Карта --> Показывать уровень зума`)
...
.. note::
   Чтобы воспользоваться этой опцией, необходимо сначала разрешить приложению запрашивать доступ к местоположению устройства в настройках устройства (:menuselect:`Настройки -> Конфиденциальность -> Местоположение`) и включить определение местоположения.
