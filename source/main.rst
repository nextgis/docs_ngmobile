...
* |ic_ruler_round| Кнопка линейки измерений (включается: :menuselection:`Настройки --> Карта --> Линейка измерений`);
* Шкала масштаба (в левом нижнем углу, включается: :menuselection:`Настройки --> Карта --> Шкала масштаба`).

В нижней части Главного окна приложения может размещаться **Панель статуса** (настроить отображение панели статуса можно в: :menuselection:`Настройки -> Карта -> Показать панель статуса`). В зависимости от размера экрана Панель статуса может занимать одну или две строки.
...
В Панели статуса отображается следующая информация (если определено местоположение):

* координаты (широта и долгота);
* текущий уровень приближения (включается в :menuselection:`Настройки --> Карта --> Показывать уровень зума`)
...
Операции над слоями вынесены в `контекстное меню слоя <https://docs.nextgis.ru/docs_ngmobile/source/main.html#ngmob-layer-menu>`_ |ic_menu_grey|. 
...
Содержимое меню слоя зависит от его типа (векторный или растровый).
...
* `Поделиться <https://docs.nextgis.ru/docs_ngmobile/source/share.html>`_;
...
Для того чтобы открыть Таблицу объектов, откройте панель Дерева слоев |ic_layer_tree|, вызовите контекстное меню |ic_menu_grey| нужного слоя и выберите **Таблица объектов**. 
...
1. |ic_back| вернуться на главный экран;
2. название слоя; 
3. |ic_search| поиск;  
4. |ic_cancel| снять выделение; 
5. ID текущего объекта; 
6. |ic_search| показать выделенный объект на карте (Если ваши объекты расположены плотно, сначала приблизьте карту так, чтобы на экран помещалось только несколько объектов, затем выберите нужный в таблице и нажмите на лупу. Карта будет перемещена к объекту с сохранением выбранного зума); 
7. |ic_delete| удалить выделенный объект; 
8. |ic_edit| открыть форму редактирования атрибутов объекта.
...
.. note::
   Чтобы воспользоваться этой опцией, необходимо сначала разрешить приложению запрашивать доступ к местоположению устройства в настройках устройства (:menuselection:`Настройки -> Конфиденциальность -> Местоположение`) и включить определение местоположения.
...
Короткое нажатие на объект открывает меню внизу экрана, где единственным активном инструментом будет |ic_info|. Нажмите его, чтобы посмотреть значения атрибутов объекта и его вложения. 
