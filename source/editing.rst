...
1. **Долгое нажатие** на объект на экране активирует режим редактирования слоя, в котором содержится данный объект. 

(Короткое нажатие также вызовет панель инструментов, но из неё можно будет только открыть информацию об объекте.) Если в выбранном месте находится несколько объектов, например, полигон и точка, то появится список, в котором следует выбрать нужный слой.

2. При нажатии на **большую синюю кнопку** в правом нижнем углу открывается меню основных операций (см. :numref:`ngmobile_edit_menu_pic`).

.. figure:: _static/ngm_main_button_ru.png
...
.. figure:: _static/ngm_main_edit_ru.png
...
3. В Дереве слоев вызовите **меню векторного слоя** и выберите пункт "Редактировать".
...
Такой вид Панели инструментов редактирования - общий для всех векторных слоев вне зависимости от типа геометрии (точка, линия или полигон).
...
Активируйте режим редактирования из контекстного меню слоя или через большую синюю кнопку в правом нижнем углу, см.  :ref:`ngmobile_switch_to_edit`. 

Когда панель инструментов редактирования откроется, нажмите кнопку |ic_add_feature|.

Процесс создания геометрий несколько отличается в зависимости от типа геометрии слоя:
...
1. Для добавления в слой точки следует выбрать из дерева слоев векторный слой с точечным/мультиточечным типом геометрии. Затем перейти в режим редактирования с помощью одного из методов, описанных в разделе :ref:`ngmobile_switch_to_edit`.

2. На панели инструментов редактирования необходимо нажать на кнопку "Добавить новый объект" |ic_add_feature|.

3. На карте появится новая точка, которая будет создана в центре экрана и подсвечена красным цветом, рядом с точкой будет отображена стрелка. Эту точку можно переместить в любое место на карте, нажав на стрелку и, не отпуская, перетянув ее на новое место. 
...
При этом доступны следующие кнопки:

* |ic_cancel| завершение режима редактирования (без сохранения); 
