...
.. warning::
   Для создания слоев из внешних геосервисов необходимо подключениt к сети Интернет.
...
2. Уточните имя слоя и его URL. URL слоя должен определять положение по оси X (номер тайла по горизонтали), Y (номер тайла по вертикали) и Z (степень приближения). Эти значения определяются с использованием универсального кода для X - **{x}**, для Y - **{y}**, для Z - **{z}**. Дополнительно можно уточнить субдомены (например, для субдоменов a.tileopenstreetmap.org, b.tileopenstreetmap.org, c.tileopenstreetmap.org адрес будет выглядеть следующим образом: **{a,b,c}.tile.openstreetmap.org**).

.. note::
   NextGIS Mobile запрашивает тайлы для каждого URL (субдомена) по двум потокам. Таким образом, с URL {a,b,c}.tile.openstreetmap.org тайлы будут загружаться по шести потокам.

3. Также можно уточнить тип тайлового слоя (XYZ (OSM) или TMS (OSGeo)), размер кэша TMS (без кэша, 1, 2 или 3 экрана) и идентификация пользователя (имя пользователя и пароль) в случае, когда требуется для доступа к тайлам. 
