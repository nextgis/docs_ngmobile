...
1. На панели инструментов дерева слоев нажать на кнопку "Добавить геоданные" (:numref:`ngmobile_layer_tree_pic`, п.4), далее выбрать пункт диалога «Создать слой» (см. :numref:`ngmobile_the_menu_button_new_layer_pic`) 
...
NextGIS Mobile позволяет создать векторный слой путем загрузки существующего файла :term:`GeoJSON`. Для этого необходимо выполнить 
...
NextGIS Mobile также позволяет создавать растровый слой путем импорта тайлового кэша в формате NGRC. Файлы NGRC могут быть получены из геоданных в практически любом формате, используя модуль `QTiles <https://docs.nextgis.ru/docs_ngqgis/source/qtiles.html>`_ для NextGIS QGIS.
