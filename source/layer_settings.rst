...
* Кэш - для векторных слоёв, здесь можно перестроить кэш.
...
Можно задать для объектов слоя разный цвет, размер и т.п. в зависимости от значения атрибута.

Для этого выберите вариант отрисовки **по правилу**. Окно настроек стиля изменится:
...
Для начала следует выбрать поле, по значению которого будут созданы правила. 

Затем нажмите **Новый**. Отроется список уникальных значений поля атрибутов, выбранного ранее. Выберите то значение, для которого хотите создать правило. Откроется диалог настройки стиля.
...
В этом диалоге задаются те же настройки, которые были описаны выше в разделе :ref:`ngmobile_simple_rendering`. Когда стиль настроен, нажмите **OK**. 

Так можно создать стили с отрисовкой по правилу для каждого значения выбранного поля атрибутов. Если для значения не задано конкретное правило, применяется вариант стиля по умолчанию, его можно настроить, нажав кнопку **По умолчанию**.
